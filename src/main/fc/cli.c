...
static void cliMap(char *cmdline)
{
    ...
    if (len == MAX_MAPPABLE_RX_INPUTS) {
        ...
        for (uint32_t i = 0; i < MAX_MAPPABLE_RX_INPUTS; i++) {
            ...
            cmdline[i] = cmdline[i] == '3' ? rcChannelLetters[6] : cmdline[i];  //woga65: AUX3 => (C)ollective pitch
            cmdline[i] = cmdline[i] == '4' ? rcChannelLetters[7] : cmdline[i];  //woga65: AUX4 => (G)yro gain
#endif            
        }
        for (uint32_t i = 0; i < MAX_MAPPABLE_RX_INPUTS; i++) {
